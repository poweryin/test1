...
def addTextTOImage1(imageSrc, newImage):
    ...
    # font = ImageFont.truetype("C:\\WINDOWS\\Fonts\\SIMYOU.TTF", 20)
    # draw.text((5, 5), "Anomaly!!!", (255), font=font)
    draw.text((5, 30), "GT-Anomaly!!!", (75,200,86))
    ImageDraw.Draw(image)
    # 保存图片
    image.save(newImage)
...
def npyLoad():
    ...
    videoPath = "/data/UCF_Crimes/test_addtxt/"
    ...
    for item in txt_list:
        ...
        pathNew = os.path.join(videoPath,  video)

        if not os.path.exists(pathNew):
            os.mkdir(pathNew)


        gt_tmp=[0]*total_frames
        x1, y1 = gt.pop(0), gt.pop(0)
        flag = True
        for i in range(len(tmp)):
            if i >= x1 and i <= y1:
                gt_tmp[i] = 1
                flag = True
            elif i > y1 and flag and gt:
                x1, y1 = gt.pop(0), gt.pop(0)
                flag = False
            else:
                pass
        ...
        image = os.listdir(os.path.join(videoPath, video))
        image.sort(key=lambda i: int(re.match(r'(\d+)', i).group()))
        pathNew = os.path.join(donePath_, video)

        if not os.path.exists(pathNew):
            os.mkdir(pathNew)

        for x, y in zip(gt_tmp, image):
            # for x,y in enumerate(image):
            image_ = videoPath + video + "/" + y
            if x == 0:
                shutil.copyfile(image_, os.path.join(pathNew, y))
            else:
                addTextTOImage1(image_, os.path.join(pathNew, y))

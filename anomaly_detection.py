...
import datetime
import os
# extract anomaly box and frame
colour=((0, 205, 205),(154, 250, 0),(34,34,178),(211, 0, 148),(255, 118, 72),(137, 137, 139))#定义矩形颜色
video_file="/home/z840/dataset/UCF_Crimes/test_demo/Anomal_box_detect/bg_video/RoadAccidents132_x264.mp4"
out_path="/home/z840/dataset/UCF_Crimes/test_demo/Anomal_box_detect/bg_box"
cap = cv2.VideoCapture(video_file) #参数为0是打开摄像头，文件名是打开视频

fgbg = cv2.createBackgroundSubtractorMOG2()#混合高斯背景建模算法

fourcc = cv2.VideoWriter_fourcc(*'XVID')#设置保存图片格式
out = cv2.VideoWriter(datetime.datetime.now().strftime("%A_%d_%B_%Y_%I_%M_%S%p")+'.avi',fourcc, 10.0, (320,240))#分辨率要和原视频对应
...
while True:
    ...
    for cont in contours:
        ...
        print("{}-prospect:{}".format(count,Area)) #打印出每个前景的面积

        rect = cv2.boundingRect(cont) #提取矩形坐标

        print("x:{} y:{} w={} h={}".format(rect[0],rect[1],rect[2],rect[3]))#打印坐标

        cv2.rectangle(frame,(rect[0],rect[1]),(rect[0]+rect[2],rect[1]+rect[3]),(0, 255, 0), 2)#原图上绘制矩形
    ...
    out.write(frame)
    cv2.imwrite(os.path.join(out_path,"{}.jpg".format(count)), frame)

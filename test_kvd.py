...
class KSVD(object):
    ...
    def fit(self, img):
        ...
        patch_num=int((256/8)**2)
        for patch_index in range(patch_num):
            #按先行后列，将图片分解成32*32个8*8的小块并装换为列向量
            r=int((patch_index/32)*8)
            c=int((patch_index%32)*8)
...
#读入原图
ori = cv2.imread("/home/z840/dataset/UCF_Crimes/crop_test_out_30/bg/Arson016_x264/1103.jpg",0).astype(np.float)
#像素丢失后的图
img=pixel_miss(ori)
#训练字典所用的图
train = cv2.imread("/home/z840/dataset/UCF_Crimes/crop_test_out_30/bg/Arson016_x264/1457.jpg",0).astype(np.float)
